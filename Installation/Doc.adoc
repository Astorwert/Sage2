...
Zde vidíme 3 různé způsoby konfigurace systému `Assisted`, `Basic`, `Expert`. Proces konfigurace pro všechny tři spočívá ve změně souboru json.

Možnosti `Assisted` a `Basic` tento proces pouze zjednodušují pro nové uživatele.
